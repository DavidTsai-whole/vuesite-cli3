<template>
  <div>
    ...
    <div class="text-right mt-4">
      <button class="btn btn-primary" data-toggle="modal" @click="openModal(true)">
        建立新的優惠卷
      </button>
      ...
      <tbody>
        <tr v-for="item in products" :key="item.id">
          ...
          <td>
            ...
            <button class="btn btn-outline-primary btn-sm" @click="deleteModal(item)">
              刪除
            </button>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    ...
    getProducts (page = 1) {
      // ?page=page 要取得的頁數
      const api = `${process.env.VUE_APP_APIPATH}/API/${process.env.VUE_APP_MEPATH}/admin/coupons?page=${page}`
    ...
    // 送出建立、編輯API
    updateProduct () {
      let api = `${process.env.VUE_APP_APIPATH}/API/${process.env.VUE_APP_MEPATH}/admin/coupon`
      let httpMethpd = 'post'
      const vm = this

      // 如果不是新資料，就改用修改產品API
      if (!vm.isNew) {
        api = `${process.env.VUE_APP_APIPATH}/API/${process.env.VUE_APP_MEPATH}/admin/coupon/${vm.tempProduct.id}`
        httpMethpd = 'put'
      }
    ...
    confirmDeleteModal () {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/API/${process.env.VUE_APP_MEPATH}/admin/coupon/${vm.tempProduct.id}`
